...
export default function Sidebar() {
  ...
  async function loadUserAndRole() {
    ...
    if (error) {
      // If this errors, you won't see admin menu.
              ...
              return (
                ...
                >
                  <div className="flex items-center">
                    <item.icon
                      className={cn('w-5 h-5 mr-3 transition-colors', isActive ? 'text-cyan-500' : 'text-gray-500 group-hover:text-gray-300')}
                    />
                    {item.name}
                  </div>
          ...
          {isRealAdmin && (
            ...
            >
              <div className="flex items-center space-x-3">
                <div className={cn('w-8 h-8 rounded-full flex items-center justify-center transition-colors', isAdminMode ? 'bg-red-900/30 text-red-500' : 'bg-gray-800 text-gray-400')}>
                  <UserCog className="w-4 h-4" />
                </div>
                <div>
                  <p className={cn('text-xs font-bold', isAdminMode ? 'text-red-400' : 'text-gray-300')}>{isAdminMode ? 'Admin Console' : 'User View'}</p>
