...
export default function DashboardClient() {
  ...
  // ---- Base load (auth + credits + cycles + subscriptions) ----
  useEffect(() => {
    let cancelled = false;
    ...
    async function fetchBase() {
      ...
      try {
        ...
        const u = authErr ? null : authData?.user ?? null;
        ...
        if (u) {
          ...
          for (const c of list) {
            const match = (subs as any[] | null)?.find(
              (s) => s.cycle_id === c.id && s.status === 'active'
            );
            c.userJoined = Boolean(match);
            c.userCreditsPaid = match?.credits_paid ?? 0;
          }
        } else {
          if (!cancelled) setJoinedCycleIds(new Set());
        }

        if (cancelled) return;
        setCycles(list);

        // Decide selected cycle (query param wins)
        const qp = cycleParam;
    ...
    // IMPORTANT: depend on stable primitive value, not the searchParams object
  }, [cycleParam]);

  // ---- Load cycle contents (groups + variants + predictions) ----
  useEffect(() => {
    ...
    async function fetchCycleContents(cycleId: string) {
      const { data: links, error: linkErr } = await supabase
        ...
        .select(
          ...
          jackpot_groups:group_id(
            id,status,draw_date,lock_time,end_time,prize_pool,currency,site_id,jackpot_type_id,
      ...
      if (predErr) {
        // still show variants even if preds fail
    ...
    fetchCycleContents(selectedCycleId);
  ...
  const userJoinedActive = useMemo(
    () => (activeCycle ? joinedCycleIds.has(activeCycle.id) : false),
    [activeCycle, joinedCycleIds]
  );
    ...
    <div className="space-y-8 px-4 pb-20 animate-in fade-in">
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <Link href="/jackpots">
          <Button
            variant="outline"
            className="border-gray-800 text-gray-300 hover:text-white hover:border-cyan-500/60"
          >
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-gray-900/20 p-6 rounded-3xl border border-gray-800">
            <div>
              <div className="flex items-center space-x-2 mb-1">
                <div
                  className={cn(
                    ...
                    activeCycle.status === 'won' ? 'bg-green-500' : 'bg-cyan-500'
                  )}
                />
                <span
                  className={cn(
                    'text-[10px] font-black uppercase tracking-widest',
                    activeCycle.status === 'won' ? 'text-green-500' : 'text-cyan-500'
                  )}
                >
              ...
              <p className="text-gray-500 text-xs font-medium mt-1 uppercase tracking-tight">
                {activeCycle.status === 'won'
                  ? 'Analysis finalized - Goal Achieved'
                  : `Cycle contains ${cycleGroups.length} jackpot groups`}
              </p>
            </div>
          ...
          {user ? (
            <div className="bg-gray-900/20 p-4 rounded-2xl border border-gray-800 flex items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-sm font-black text-white">
                    {userJoinedActive ? 'Access granted' : 'Predictions locked'}
                  </div>
                  <div className="text-[11px] text-gray-500">
                    {userJoinedActive
                      ? 'You can view the full predictions for this cycle.'
                      : `Join to unlock. Credits: ${credits}`}
                  </div>
                </div>
              </div>
              ...
              {!userJoinedActive && (
                <Button
                  onClick={handleJoin}
                  className="bg-cyan-500 hover:bg-cyan-400 text-black font-black rounded-xl px-6"
                >
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <div className="text-xs text-gray-500 font-black uppercase tracking-widest flex items-center">
                        <Calendar className="w-3 h-3 mr-2" /> Locks:{' '}
                        {group.lock_time ? new Date(group.lock_time).toLocaleString() : '—'}
                      </div>
                      ...
                      <div className="text-[11px] text-gray-500">
                        Status: {group.status}
                        {group.prize_pool != null
                          ? ` • Pool: ${Number(group.prize_pool).toLocaleString()} ${group.currency || 'KES'}`
                          : ''}
                      </div>
                    </div>
          ...
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 font-medium">
                Create an account to unlock predictions and track outcomes.
              </p>
            </div>
          </div>
          <Link
